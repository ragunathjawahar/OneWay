...
class SmileyFragment : OneWayFragment<SmileyState>(), SmileyTransientViewDriver {
  companion object {
  ...
  override fun source(
    ...
    SmileyModel.createSource(
        ...
        SmileyUseCases(initialState, sourceCopy, this)
    )
  ...
  override fun pickSmileyCancelled() {
    Toast
        .makeText(context, R.string.pick_smiley_cancelled, Toast.LENGTH_SHORT)
        .show()
  }
  ...
  override fun onStart() {
    ...
    compositeDisposable += resultDispatcher
        .result<String>()
        .map { if (it == SMILEY_NONE) PickSmileyIntention.NONE else PickSmileyIntention.of(it) }
